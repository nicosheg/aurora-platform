    ...
    <main className="relative min-h-screen bg-[#0d0a05] text-[#fff8ee]">
      ...
      <AnimatePresence mode="wait">
        {currentWorld === "opening" && (
          <StoryOpening name={name} onComplete={() => goTo("one")} morningUnlocked={morningUnlocked} />
        )}
        {currentWorld === "one" && morningUnlocked && (
          <TinyPrincess photos={chapters.one.photos} onNext={() => goTo("two")} />
        )}
        ...
        {currentWorld === "three" && afternoonUnlocked && (
          <TreasureHunt onNext={() => goTo("four")} />
        )}
        {currentWorld === "four" && afternoonUnlocked && (
          <DreamWorld onNext={() => goTo("five")} />
        )}
        {currentWorld === "five" && eveningUnlocked && (
          <FutureLetters onNext={() => goTo("ending")} />
        )}
        {currentWorld === "ending" && eveningUnlocked && (
          <StoryEnding name={name} senderName={senderName} onNext={() => goTo("capsule")} />
        )}
        {currentWorld === "capsule" && eveningUnlocked && (
