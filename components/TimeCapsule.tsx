    ...
    <motion.section initial={{ opacity:0 }} animate={{ opacity:1 }} exit={{ opacity:0 }} className="min-h-screen flex flex-col items-center justify-center px-4 py-20 text-center bg-black">
      ...
      {!sealed ? (
        <div className="w-full max-w-md">
          ...
          <textarea className="w-full h-32 p-4 rounded-xl bg-white/5 border border-[#d4a574]/30 text-white/80 font-serif" placeholder="Dear future me..." value={message} onChange={e => setMessage(e.target.value)} />
          <motion.button onClick={seal} className="mt-4 px-8 py-3 bg-gradient-to-br from-[#d4a574] to-[#b8956e] text-white font-serif rounded-full" whileHover={{ scale:1.05 }}>Seal Time Capsule 💌</motion.button>
        </div>
      ) : (
      ...
      <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }} transition={{ delay:1 }} className="mt-12">
        <a href="https://fiduciaweb.netlify.app" target="_blank" rel="noopener noreferrer" className="fiducia-signature text-white/30">
