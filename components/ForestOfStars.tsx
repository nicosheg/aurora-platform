...
export default function ForestOfStars({ onNext }: { onNext: () => void }) {
  ...
  return (
    <motion.section initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="min-h-screen flex flex-col items-center justify-center px-4 py-20 text-center bg-gradient-to-b from-[#0a1a0a] to-[#0d0a05] relative overflow-hidden">
      <CelebrationOverlay />
      ...
      <div className="flex flex-wrap justify-center gap-6 mb-12">
        {treasures.map(t => (
          <motion.button key={t.id} className={`text-5xl p-4 rounded-full transition-all ${found.includes(t.id) ? "bg-[#d4a574]/40" : "glass-card-light"}`} whileHover={{ scale: 1.2 }} whileTap={{ scale: 0.8 }} onClick={() => discover(t.id)}>
            {t.emoji}
          </motion.button>
        ))}
      </div>
