...
export default function StoryOpening({ name, onComplete, morningUnlocked }: { name: string; onComplete: () => void; morningUnlocked: boolean }) {
  ...
  return (
    <motion.div
      ...
      style={{ background: "radial-gradient(ellipse at center, #1a1208 0%, #0d0a05 100%)" }}
    ...
    >
      <div className="absolute inset-0 overflow-hidden">
        {[...Array(40)].map((_, i) => (
          <motion.div
            ...
            animate={{ opacity:[0,0.8,0], scale:[0,1.5,0], y:[0,-100] }}
      ...
      <div className="relative z-10 text-center px-6 max-w-2xl">
        <AnimatePresence mode="wait">
          ...
          {step===2 && (
            <motion.p key="s2" initial={{ opacity:0, y:40 }} animate={{ opacity:1, y:0 }} exit={{ opacity:0 }} className="text-xl md:text-3xl font-serif font-light text-[#f0d080]">
              And yours started on a very special day.
            </motion.p>
          )}
          {step===3 && (
            <motion.p key="s3" initial={{ opacity:0, y:40 }} animate={{ opacity:1, y:0 }} exit={{ opacity:0 }} className="text-2xl md:text-4xl font-serif font-light text-[#f0d080]">
          ...
          {step>=4 && (
            <motion.div key="s4" initial={{ opacity:0, scale:0.95 }} animate={{ opacity:1, scale:1 }} className="space-y-10">
              ...
              <h1 className="text-4xl md:text-6xl font-serif font-light" style={{ background:"linear-gradient(135deg, #f0d080 0%, #d4a574 100%)", WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent" }}>
                Happy Birthday,<br/>{name}
              </h1>
              {morningUnlocked ? (
                <motion.button onClick={handleBegin} className="px-10 py-4 bg-gradient-to-br from-[#d4a574] to-[#b8956e] text-white font-serif text-lg rounded-full shadow-2xl" whileHover={{ scale:1.04 }} whileTap={{ scale:0.96 }}>Enter Your Story</motion.button>
              ) : (
