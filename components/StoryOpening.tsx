...
export default function StoryOpening({ name, onComplete, morningUnlocked }: { name: string; onComplete: () => void; morningUnlocked: boolean }) {
  ...
  useEffect(() => {
    const t0 = setTimeout(() => setStarted(true), 800);
  ...
  useEffect(() => {
    ...
    if (currentPage < pages.length) {
      const timer = setTimeout(() => setCurrentPage(prev => prev + 1), 5500); // slower: 5.5s per page
      return () => clearTimeout(timer);
    } else {
      const timer = setTimeout(() => setShowFinal(true), 3000); // pause before final
      return () => clearTimeout(timer);
    }
  }, [currentPage, started]);
        ...
        <AnimatePresence>
          ...
          {showFinal && (
            ...
            >
              ...
              <p className="text-[#4a3728]/70 font-serif italic text-lg mb-10">
