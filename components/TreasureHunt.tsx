  ...
  return (
    <motion.section initial={{ opacity:0 }} animate={{ opacity:1 }} exit={{ opacity:0 }} className="min-h-screen flex flex-col items-center justify-center px-4 py-20 text-center">
      ...
      <div className="flex flex-wrap justify-center gap-6 mb-12">
        {treasures.map(t => (
          <motion.button key={t.id} className="text-5xl p-4 glass-card-light rounded-full" whileHover={{ scale:1.2 }} whileTap={{ scale:0.8 }} onClick={() => discover(t.id)}>
            {t.type}
          </motion.button>
        ))}
      </div>
      ...
      {found.length === 4 && (
        <motion.button onClick={onNext} className="px-8 py-3 bg-[#d4a574]/20 border border-[#d4a574]/40 rounded-full text-[#f0d080]" whileHover={{ scale:1.05 }}>Continue to Dream World</motion.button>
      )}
    </motion.section>
  );
}
